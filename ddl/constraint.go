...
func checkTooLongConstraint(constraint model.CIStr) error {
	if len(constraint.L) > mysql.MaxConstraintIdentifierLen {
		return ErrTooLongIdent.GenWithStackByArgs(constraint)
	}
	return nil
}
