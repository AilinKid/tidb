...
func (d *ddl) CreateEvent(ctx sessionctx.Context, s *ast.CreateEventStmt) error {
	ident := ast.Ident{Schema: s.EventName.Schema, Name: s.EventName.Name}
	...
	// to genGlobalID like them here. EventID can be generated by the system table's
	...
	eventInfo := &model2.EventInfo{
		...
		Enable:     model2.FormEventEnableType(s.Enabled.String()),
	...
	// build the execute instance info.
	instance := ""
	switch s.Instance {
	case ast.EventInstanceAny:
	case ast.EventInstanceCurrentOnly:
		instance = d.uuid
	case ast.EventInstancePlacementConstraints:
		instance = s.InstancePlacementConstraints
	default:
	}
	...
	if startTime.GetMysqlTime().Compare(endTime.GetMysqlTime()) == 0 {
		...
		if s.Schedule.IntervalValue != nil {
			...
			// TODO: interval can't be evaluated as a number.
